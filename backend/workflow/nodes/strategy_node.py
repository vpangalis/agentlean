...
# DEPRECATED: replaced by strategy_node() function above — remove in Phase 8
class StrategyNode:
    ...
    def run(
        self, question: str, country: str | None,
        model_name: str | None = None, state: dict[str, Any] | None = None,
    ) -> StrategyNodeOutput:
        ...
        formatted_cases = json.dumps(
            [c.model_dump(mode="json") for c in all_cases], indent=2, default=str,
        )
        formatted_knowledge = "\n".join(
        ...
        if strategy_escalated and strategy_fail_section and strategy_response:
            system_prompt = StrategyNode._ESCALATION_SYSTEM_PROMPT.format(
                fail_section=strategy_fail_section, fail_reason=strategy_fail_reason,
                original_response=strategy_response, formatted_cases=formatted_cases,
            )
            user_prompt = f"USER QUESTION: {question}"
        else:
        ...
        return StrategyNodeOutput(
            strategy_draft=StrategyPayload(
                summary=response_text, supporting_cases=all_cases,
                supporting_knowledge=knowledge_docs, suggestions=suggestions,
            )
        )
        ...
        try:
            ...
            for line in section.split("\n"):
                ...
                if line.upper().startswith("TEAM:"):
                    q = line[5:].strip().strip('"')
                    if q:
                        suggestions.append({"label": (q[:40] + "..." if len(q) > 40 else q), "question": q, "type": "team"})
                elif line.upper().startswith("COSOLVE:"):
                    q = line[8:].strip().strip('"')
                    if q:
                        suggestions.append({"label": (q[:40] + "..." if len(q) > 40 else q), "question": q, "type": "cosolve"})
        except Exception:
            pass
        return suggestions
