...
# DEPRECATED: replaced by kpi_reflection_node() function above — remove in Phase 8
class KPIReflectionNode:
    ...
    def run(self, question: str, metrics: KPIResult) -> KPIReflectionOutput:
        ...
        if audit.should_regenerate:
            interpretation = self._generate_interpretation(
                question, metrics, issues=audit.issues, use_regeneration_client=True,
            )
        all_issues = audit.issues[:]
        if not audit.scope_correct:
            all_issues.append(f"Scope issue: {audit.scope_feedback}")
        ...
        if not audit.data_grounded:
            all_issues.append(f"Grounding issue: {audit.grounding_feedback}")
        if audit.banned_terms_found:
            all_issues.append(f"Banned terms in output: {', '.join(audit.banned_terms_found)}")
        ...
        return KPIReflectionOutput(
            kpi_interpretation=KPIInterpretation(
                summary=interpretation.summary, insights=interpretation.insights,
                metrics=metrics, render_hint=metrics.render_hint,
                scope_label=metrics.scope_label, suggestions=metrics.suggestions,
            ),
            kpi_reflection=verdict,
        )

    def _generate_interpretation(
        self, question: str, metrics: KPIResult,
        issues: list[str] | None = None, use_regeneration_client: bool = False,
    ) -> KPIInterpretationDraft:
        client = self._regeneration_llm_client if use_regeneration_client else self._llm_client
        issues_text = f"\nYou must address these quality issues: {issues}" if issues else ""
    ...
    def _semantic_audit(
        self, question: str, metrics: KPIResult, interpretation: KPIInterpretationDraft,
    ) -> KPISemanticAudit:
        suggestions_text = "\n".join(f"  {i+1}. {s}" for i, s in enumerate(metrics.suggestions))
        return self._llm_client.with_structured_output(KPISemanticAudit).invoke([
            SystemMessage(content=KPI_REFLECTION_STEP2_PROMPT),
            HumanMessage(content=(
                f"User question: {question}\nScope used: {metrics.scope}\n"
                f"render_hint used: {metrics.render_hint}\nSuggestions:\n{suggestions_text}\n"
                f"responsible_leader: {metrics.responsible_leader!r}\n"
                f"department: {metrics.department!r}\n"
                f"Summary produced: {interpretation.summary}\n"
