...
# DEPRECATED: replaced by operational_node() function above — remove in Phase 8
class OperationalNode:
    ...
    def run(
        ...
        if is_new_problem_question(question, case_id):
            ...
            if knowledge_docs:
                ...
                if explore_marker in response_text:
                    idx = response_text.index(explore_marker)
                    response_text = response_text[:idx].rstrip() + knowledge_section + "\n\n" + response_text[idx:]
                else:
            ...
            return OperationalNodeOutput(
                operational_draft=OperationalPayload(
                    ...
                    current_state_recommendations=response_text,
                    next_state_preview="",
                    supporting_cases=[], referenced_evidence=[], suggestions=suggestions,
                )
            )

        if case_status == "closed" and case_id:
            supporting_cases = self._hybrid_retriever.retrieve_similar_cases(
                query=question, current_case_id=case_id, country=self._extract_country(case_context),
            )
            referenced_evidence = self._hybrid_retriever.retrieve_evidence_for_case(case_id=case_id)
            user_prompt = (
                f"CLOSED CASE: {case_id}\nUSER QUESTION: {question}\n"
                "\n--- CASE HISTORY ---\n" + format_d_states(case_context) + "\n"
                "\n--- SUPPORTING CLOSED CASES ---\n"
                + json.dumps([item.model_dump(mode='json') for item in supporting_cases], indent=2) + "\n"
                "\n--- REFERENCED EVIDENCE ---\n"
                + json.dumps([item.model_dump(mode='json') for item in referenced_evidence], indent=2)
            )
            user_prompt = user_prompt + "\n--- KNOWLEDGE BASE REFERENCES ---\n" + knowledge_block
            response_text = llm.invoke([
        ...
        user_prompt = self._build_structured_prompt(
            case_context=case_context, case_id=case_id, question=question,
            current_state=current_state, supporting_cases=supporting_cases,
            referenced_evidence=referenced_evidence,
        )
        user_prompt = user_prompt + "\n--- KNOWLEDGE BASE REFERENCES ---\n" + knowledge_block
        response_text = llm.invoke([
    ...
    def _build_structured_prompt(
        self, case_context: dict[str, Any], case_id: str, question: str,
        current_state: str, supporting_cases: list[Any], referenced_evidence: list[Any],
    ) -> str:
        formatted_d_states = format_d_states(case_context)
        formatted_supporting_cases = json.dumps(
