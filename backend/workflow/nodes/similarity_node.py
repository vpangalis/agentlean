...
# DEPRECATED: replaced by similarity_node() function above — remove in Phase 8
    ...
    def run(
        ...
        if knowledge_docs:
            ...
            if explore_marker in response_text:
                idx = response_text.index(explore_marker)
                response_text = (
                    response_text[:idx].rstrip() + knowledge_section + "\n\n" + response_text[idx:]
                )
            else:
                response_text = response_text + knowledge_section
        suggestions = extract_similarity_suggestions(response_text)
        return SimilarityNodeOutput(
            similarity_draft=SimilarityPayload(
                summary=response_text, supporting_cases=cases, suggestions=suggestions,
            )
        )
