...
def build_step_board(cwd: str, today: str | None = None) -> str:
    """The four-state step board, for REFACTORING_PROCEDURE.md.

    **DONE · BUILDING NOW · BLOCKED · QUEUED**, derived from exactly three
    things and nothing else:

        DONE          a `refactor(arch-v2): commit X.Y` subject exists
        BLOCKED       the Appendix D row carries BLOCKED / GATED / EXTERNAL
        BUILDING NOW  the lowest available row above the highest DONE
        QUEUED        everything else

    **It deliberately does NOT render `| **Commit X.Y** |` rows.** Both readers
    bound their parse to the Appendix D section, so a second table would be
    safe today — but "safe because of where it happens to sit" is the kind of
    guarantee that breaks when someone moves a heading. A different shape
    cannot be mistaken for the step index at all.
    """
    rows = parse_step_index(staged_text(PROCEDURE, cwd))
    landed = set(spine_steps(cwd))
    date = today or _dt.date.today().isoformat()
    ...
    for step, title, status in rows:
        if step in landed:
            ...
            blocked.append((step, title, status.upper()))
        elif step != nxt:
            queued.append((step, title))

    def names(pairs):
        return ", ".join(f"**{s}**" for s, *_ in pairs) or "—"
    ...
    out = [
        ...
        f"| State | Count | Steps |",
        f"|---|---|---|",
        f"| **DONE** | {len(done)} | {names(done)} |",
        f"| **BUILDING NOW** | {1 if nxt != '—' else 0} | "
        f"**{nxt}** — {t['next_what']} |",
        f"| **BLOCKED** | {len(blocked)} | "
        + (", ".join(f"**{s}** ({k})" for s, _, k in blocked) or "—") + " |",
        f"| **QUEUED** | {len(queued)} | {names(queued)} |",
        "",
        f"*{len(rows)} rows. DONE is git history — the "
        "`refactor(arch-v2): commit X.Y` subjects, intersected with this table, "
        "so a step that landed under another subject is not counted. BLOCKED is "
        "Appendix D's status column, the only thing git cannot say. "
        f"Regenerated {date}.*",
        BOARD_END,
    ]
    return "\n".join(out)
