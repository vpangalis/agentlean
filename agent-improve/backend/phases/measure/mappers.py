"""Measure's boundary mappers — procedure step 3.3.
...
def measure_input_mapper(parent: SupervisorState, store: BaseStore) -> PhaseState:
    """`SupervisorState` → `PhaseState` for Measure.

    Reads define's approved gate document from the Store and frames this
    ...
    **A missing prior gate document raises** rather than defaulting: it means
    the define gate never applied, which is an ordering fault, and S-C06's
...
__all__ = ["measure_input_mapper", "measure_output_mapper",
