...
IMPROVE_REQUIRED_FOR_GATE = list(IMPROVE_TIER_1_FIELDS)
...
async def validate_improve(state: ImproveGraphState) -> dict:
    ...
    missing: list[str] = [
        field for field in IMPROVE_REQUIRED_FOR_GATE if _is_empty(data.get(field))
    ]

    # -- Sub-field completeness for the structured dicts (41) ----------
    missing.extend(_missing_structured(data))

    # Value-domain check: a bare "no" does not pass the gate.
    ...
    if passed:
        logger.info(
            "Improve gate PASSED — %d acknowledged Tier 2 gap(s): %s",
            len(gaps), gaps,
        )
        return {
            "gate_attempts": 0,
