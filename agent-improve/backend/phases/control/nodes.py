"""The five nodes of the Control phase subgraph.
...
`orchestrate_control` the executor delegates to, and which `validate_control`
...
Control's orchestrator builds a **cross-phase brief from every prior
phase** on every call, so it reads all their `phase_inputs` as well as
its own.
...
Control gate is inert**: `validate_control` requires the §39.x v2 names and the
...
async def executor(
    ...
    """Run one coaching turn, delegating to `orchestrate_control` (§17)."""
    return await _c.executor(PHASE, orchestrate_control, state, config)
...
async def validation_stack(
    ...
    """The four layers, shared cap of 3 (§34) — layer 2b via `validate_control`."""
    return await _c.validation_stack(PHASE, validate_control, state, config)
