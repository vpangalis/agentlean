...
| **6** | **Tools and knowledge** — §23–§32 | **26 / 28 tools · 3 / 3 indexes** | ✅ 3 `rag_lookup_*` + `propose_template` + `propose_diagram` · ☐ `check_gate_status` (7.1) ☐ `request_human_approval` (7.5) — WATCH 25 — ✅ `load_evidence_series` (**built 6.12** — re-parses the blob, never a stored copy; the set became eight on 2026-09-09, DECISIONS Part AR) · ✅ 20 computation tools, bound 1/8/5/1/5 per phase · ✅ **the upload path is built (6.11)** — csv/xlsx/pdf/docx parse deterministically, an unreadable file is refused before the blob write, evidence indexes and artefacts do not · ✅ **`improve_evidence_index` carries all twelve fields as of 6.13** — the seven ratified-not-yet-applied are live, filterable and populated on every surviving document; retrieval defaults to `kind eq 'evidence'` and `rag_lookup_evidence` returns §24's structured record. **The step is NOT closed: its `live-run` half is blocked by a pre-existing executor loop** that reproduces identically on 6.12 · ✅ `load_skill` via middleware 2 · ✅ **5 of 5 SKILL.md files exist, load, and carry all seven of §32's mandatory items** — Define's A→F flow, Uploads and §43.7 metric literacy landed at 6.9. All five byte-match their §39.x.10 section |
...
|---|---|---|---|
| 1 | `BeforeModelStateInjection` | `before_agent` composes · `wrap_model_call` prepends | ✅ **as of 6.12 it also injects the UPLOAD MANIFEST** — §6's "evidence context" reader, declared since 3.1 and wired to nothing until then (WATCH-19's third instance, Part AS3). ✅ **as of 6.8 it injects `phase_context`.** Was ⚠️ from 6.3 to 6.8 — composed the block without it |
