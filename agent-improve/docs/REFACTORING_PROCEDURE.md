...
| 582 | **Commit 6.38** | The dual-read is removed — one register, one reader |  | OPS | SHARED | Both documents stay readable as the register, which is the two-sources-of-truth condition the repartition was performed to end. |
| 583 | **Commit 11.2** | Governance close-out |  | OPS | SHARED | The refactor has no end, so procedure and architecture drift apart again with nothing marking the handover. |
