...
| L4 |  | COACH | **11.1** | the five `{PHASE}_COACH_PROMPT` carry the memory-hierarchy block and the anti-hallucination guards (6.6). The v1 `EXTRACTION_{PHASE}` family survives only in `core/prompts.py` and the five v1 `orchestrate.py`, which **step 11.1 deletes wholesale** | ✅ | — | §22 |
...
| L8 |  | STORE | — | `storage/blob.py` is module-level `async` functions since 3.5; the class is gone and `grep -rn "class ImproveBlobClient"` returns zero | ✅ | — | §10 |
