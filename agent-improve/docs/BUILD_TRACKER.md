...
**Progress: 30 of 55 build steps done** (2.3–2.7, 3.1–3.5, 4.1–4.4, 5.1–5.4, 6.1–6.9, 6.11–6.12 + 9.0 out-of-band). **Next: step 6.13 — 6.10 is BLOCKED. STAGE 6 IS NOT COMPLETE — the 2026-09-07 audit reopened it.**
...
| 6.13 | **The evidence index migration** | §23.2, §23.2.1, §23.4, §24, S-C02 | ☐ ▶ **next** *(applies §23.2's seven RATIFIED-NOT-YET-APPLIED fields — `phase`, `uploaded_at` and the five ratified 2026-09-09. **Four sub-steps and the order is load-bearing**: schema add → write path → backfill from the case blobs → retrieval filters. Filters before the write path expose artefacts with nothing hiding them; backfill before the schema has nothing to write into. **No drop-and-rebuild** — adding a field to a live index is additive and assigns `null` to existing documents, which is why this is a step and not a rider on 9.1. `fields=EVIDENCE_INDEX_FIELDS` is not optional (§23.4). Not a precondition of 6.12; anything uploaded between them is caught by the backfill. DECISIONS Part AQ)*  |
