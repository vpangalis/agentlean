...
# updated 2026-09-10 · build target: `agent-improve/ARCHITECTURE.md`

**Progress: 31 of 55 build steps done** (2.3–2.7, 3.1–3.5, 4.1–4.4, 5.1–5.4, 6.1–6.9, 6.11–6.13 + 9.0 out-of-band). **Next: step 6.14 — 6.10 is BLOCKED. STAGE 6 IS NOT COMPLETE — the 2026-09-07 audit reopened it.** ⛔ **A LIVE DEFECT NOW SITS UNDER FOUR STEPS' `live half owed`:** the executor loops on `rag_lookup_evidence` until its 45s node timeout instead of calling the `load_evidence_series` its own planner named, so no Define turn on `IMPR-2026-ED8` completes. Found at 6.13, reproduced identically on 6.12's code, so it is neither step's doing — see 6.13's section.
...
half was step 6.13 and **landed 2026-09-10**). Everything else is open once its precondition step lands.
...
| 6.9 | SKILL.md conformance to §32's seven | §32, §43, §37 | ✅ done *(**the step was the opposite file.** All five existed; the four "drafts" carried 7/7 of §32's items and **Define — recorded as the finished one — carried 4.5/7**. Written into Define: the A→F session flow with its `n of 12` count, §43.7 metric literacy, and Uploads + capture as real sections. 21.8K → 32.0K chars. Metric literacy landed in §39.1.7 too — v1.17 embeds coaching script, not flow or capture — and containment re-verified byte-exact. Version labels aligned and their meaning declared: Define 1.2, the four 1.1, "0.2-draft" false since v1.17. **Live re-run still owed — blocked on Azure 429s (AI2).** DECISIONS Part AO)* |
...
| 6.11 | **The upload path (G-36)** | §29.1, §6, §10, §23.2, S-F35 | ✅ done *(**`PhaseState.uploads` has a writer.** csv/xlsx/pdf/docx parse deterministically to columns / rows / types / ranges; an unreadable file is refused with a Belt-readable reason **before** the blob write, so a refusal cannot leave a stored file nothing can use; evidence indexes and artefacts do not (ruling 3); `evidence_index_id` is captured instead of discarded; one operational-tier interpretation call, citing its source upload. `rows` gets its first writer since before the refactor. **The parse found its own defect**: Excel returns 63.0 as `63`, which split a numeric column in two and put its largest value outside the reported range — `whole number` and `decimal` now share one range. **Live-run and azure-query PASSED 2026-09-09** on `IMPR-2026-ED8`, which is kept as 6.12's fixture: four formats parsed and interpreted, `PhaseState.uploads` carrying §6's shape with `ask_id`/`version` `None`, the artefact absent from the index, and a refused file writing no blob. **The live run is what caught 6.11a's defect** — the interpretation prompt asked for prose while the parser read JSON, so every summary was the fallback and 781 green tests could not see it. DECISIONS Part AP)* |
| 6.12 | **Ask-binding: an upload answers a request** | §29.1, §6, §23.2.1, S-F57 | ✅ done *(**the planner derives the ask; the model never declares one** — a field the model must emit is absent whenever it forgets, and nothing says it should have been there (AR-R1). `role`/`shape_match`/`content_digest` on `UploadRecord`; supersession on `(case_id, role)` by digest; the upload manifest injected every turn — **§6's "evidence context" reader, declared and wired to nothing until now, WATCH-19's third instance**; `load_evidence_series` per S-F57; citations anchored in code on ROLE, never filename. **Clause 7 is half-done by ruling** — a citation from `rag_lookup_evidence` cannot be anchored until 6.13. Measure's SKILL.md only; the other four are 6.14. **Live-run owed.** DECISIONS Part AS)* |
| 6.13 | **The evidence index migration** | §23.2, §23.2.1, §23.4, §24, S-C02 | ✅ done — **live half BLOCKED, not owed** *(all seven fields live, filterable and populated; `azure-query` PASSED on every clause. **Three founder rulings were needed first** (Part AT, commit `1714d75`): the Done-when was unsatisfiable as written, and the audit that found it read the RAW stored JSON — `role`, `shape_match` and `content_digest` are ABSENT KEYS on every pre-6.12 record, and a loaded `UploadRecord` manufactures `'other evidence'` for them as a Pydantic default. Sentinel role `unclassified (pre-ask-binding)`; the backfill's source widened to the index's own metadata for one document with no case record; three superseded documents whose bytes `overwrite=True` destroyed were DELETED per §23.2 rather than given a false digest. **The `live-run` was ATTEMPTED and FAILED** — the executor loops on `rag_lookup_evidence` until the 45s node timeout instead of calling the `load_evidence_series` its planner named. **Reproduced identically on `1714d75`, so it is pre-existing**, and it invalidates the live half of 6.7, 6.9 and 6.12 too. DECISIONS Part AT)*  |
| 6.14 | **SKILL.md shape pass — the other four phases** | §32, §43, §23.2.1 | ☐ ▶ **next** *(6.12 did Measure only. **Scheduled rather than assumed**: four-fifths of a content pass silently owed is this project's recurring disease. Carries §23.2.1's vocabulary extension with it — 6.12 found no role for a measurement-system study and used the `other evidence` catch-all rather than extending the vocabulary four times. DECISIONS Part AS)* |
| 6.15 | **The count-check** | §55.1, §6, §29.2 | ☐ *(hand-written assertions that a written count matches the list it describes — §6's author-populated sentence against S-C02's rows, §29.2's universal count against its S-F entries, and the others AR3 names. **No model, no network.** After the third caption to outlive its own list; each was found by editing the thing beside it, never by a check. DECISIONS Part AR3)* |
| 6.16 | **The board is generated, not written** | §55.1, §66, Appendix D | ☐ *(**READY — nothing blocks it.** The board is hand-made in a chat and goes stale the moment a commit lands; four captions here have already outlived their lists (AR3), and a hand-written board is the fifth waiting to happen. Appendix D gains **Zone** and **Impact** — impact being *what is true about the product if this step is never done*, **the one column that exists in no document today** and the bulk of the work at 55 sentences. ARCHITECTURE_STATUS's ☐/⚠ rows get a parseable closer token so the generator reads `[6.10]` rather than a sentence, with `[none]` said out loud. `.claude/hooks/build_board.py` writes `docs/board.html` from those two plus §66 and the cursor, **reading nothing else** — four containers, and **every ☐/⚠ in the architecture diagram carries its step number**. **Lanes DERIVED, never declared**: QUEUED / BUILDING NOW / BLOCKED / READY / DONE. Pre-commit, fail-SOFT; `board.html` joins the watched paths. **⚠ Zone-to-block is NOT 1:1** — eight blocks, seven zones — settled as data before anything is drawn)* |
...
| 9.1 | The batched reindex — **case index only** | §23 | ⏸ external *(**narrowed 2026-09-09**: the evidence half moved to 6.13. This step assumed the evidence fields needed the drop-and-rebuild and were therefore external and blocked; adding a field to a live index is additive and needs neither. What remains is `embedding` → `content_vector` on the case index and the shared contextual-label re-ingest. DECISIONS Part AQ3)* |
...
| 2 | **This file AND `REFACTORING_PROCEDURE.md`** (its Appendix D) are staged in the same commit — one step = one commit = one row moved **in both**. Appendix D joined the rule on 2026-09-08: requiring only the tracker let the two disagree exactly where a tool reads one of them — after 6.8 shipped, Appendix D still carried it as `pending` and 6.9 under its pre-audit title, because nothing made the commit touch that file |
