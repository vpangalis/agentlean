...
| **G-57** | **THE TWO RULES THAT GUARD THE TREE ARE DEMONSTRATED AND UNTESTED.** Rules 7 and 8 of the commit-msg guard enforce §0.32's scratch and step-number clauses. Each is proven to fail and to pass by hand — a staged `scratch/` path and a staged `.bak` blocked by name, an undeclared new path blocked, `Step: 99.9` and `Gap: G-999` blocked as unresolvable, `Step: 6.22` and `Gap: G-52` passed — **and nothing re-runs any of it.** Rule 6 carries a test suite for exactly this reason, stated in its own file: *a message check fails SILENTLY by letting commits through*, which is the one failure mode a gate must not have. **An index check fails the same way**: a pattern list that stops matching, or a register regex that stops resolving after a table is reformatted, reports nothing and blocks nothing. Appendix D's header already warns that its format is parsed by three hooks — rule 8 is the fourth reader of that table, and the second of §66's. Registered 2026-09-13 with the rule itself rather than after it, because a gate with no test is a convention wearing a gate's clothes. **No step owns this yet** — it is a test file, so landing it is itself subject to rule 8, and the step or gap it cites is a founder call | §0.32, §56.3, §55.1 |
