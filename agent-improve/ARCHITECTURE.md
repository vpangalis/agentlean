...
| **G-54** | **THE VERIFICATION LOG'S VERDICTS ARE SOURCED FROM DOCUMENTATION PAGES, AND ONE OF THEM WAS WRONG BECAUSE OF IT.** Four of `BIBLE_VERIFICATION_LOG.md`'s five `Source` cells name a `reference.langchain.com` or `docs.langchain.com` page; none names an introspection of the installed object. **C-3 is the proven instance** — its Source was a MODULE's API page and its Claim was about a CLASS's member list, so it read what the module exports as what the class exposes, stamped CORRECTED, and overwrote a statement that had been right (withdrawn 2026-09-12, ARCHITECTURE.md v1.35). **The other three have never been re-checked.** C-1 (`retries=` vs `max_retries=`) and C-2 (`prompt=` vs `system_prompt=`) are incidentally pinned by `test_retry_kwargs_against_the_installed_classes` and `test_executor.py`'s signature assertions — by accident, not by the log. **S-1 is pinned by nothing and carries a live architectural decision**: it asserts `BaseStore.search()`'s parameter surface from two doc pages, and §9's ruling to keep `improve_case_index` on Azure AI Search now rests on the single reason that survived it. A doc page is evidence ABOUT an API; the installed object IS the API. Re-verification, in-place correction and a `Source method` column are **step 6.23**. | §9, §16.3, §25, §53, §55.1 |
