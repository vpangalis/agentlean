...
| `shape.py` | The architecture diagram, read from the code: the compiled graphs, the `create_agent` middleware list, probes of the skills and state-injection middleware |
| `stories.py` | The plan's epics, stories and tasks. No status is typed; `resolve()` derives them from `progress()` |
