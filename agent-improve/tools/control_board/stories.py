...
EPICS: list[dict[str, Any]] = [
 dict(id="E1", title="A Belt can complete Define and close its gate",
  ...
  dict(id="S33", rank=3, status="todo", rows=[25], comps=["Phase state","Coach"],
  ...
  dict(id="S4", rank=5, status="todo", rows=[14], comps=["UI"],
  ...
  dict(id="S5", rank=6, status="todo", rows=[7], comps=["Coach"],
  ...
  dict(id="S6", rank=7, status="todo", rows=[23,34], comps=["Gate","Routes","UI","Storage"],
 ...
 dict(id="E2", title="The coach teaches the Lean Six Sigma method, not just asks questions",
      ...
      stories=[
  dict(id="S40", rank=1, status="todo", rows=[2, 13], comps=["Coach","Storage"],
       ...
       bugs=[("The grader re-judges the same unchanged reply on each iteration, inside the executor's time budget — rows 2 and 13",
             ...
             ("The executor's soft budget does not end the turn before the 45 s limit — row 2, two traces",
              "Coach","done","6.49 · traces 01a0d215, 01a0d28e · fixed 6.52 B1")],
  ...
  dict(id="S7", rank=4, status="todo", rows=[3], comps=["Coach","Skills"],
 ...
 dict(id="E3", title="Every capability is proven, not assumed",
      ...
      stories=[
  dict(id="S9", rank=2, status="todo", rows=[1,2,5,6,8,10,11,12,13,17,33,35], comps=["all"],
 ...
 dict(id="E4", title="Quality, once Define works end to end",
  ...
  dict(id="S11", rank=None, status="todo", rows=[9], comps=["Coach"],
       ...
       tasks=[("6.43","A tool that returns an uploaded document's text","Coach","todo")],
       bugs=[("The same evidence file is indexed twice, 22 s apart — G-97","Storage","todo","6.46 Part A"),
             ("The evidence upload's interpretation is unavailable — G-98","Storage","todo","6.46 Part A")]),
