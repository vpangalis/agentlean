...
Every field carries four parts — Explain, Show, Ask, Read back — in **Coaching
content** below. **Which part a turn uses, and which field it is on, is decided
in code** (ARCHITECTURE.md §17, v1.75), never by this file. The order in §2 is
...
**Echo the value, show the updated count, name the field it opens.** Three beats, one
...
catch a mis-capture — the Belt reads their own words back and says "no, I meant
per month, not per quarter" before that value is stored and reaches a gate
document and six downstream fields.
...
**If nothing is thin, say so.** Manufacturing a concern to fill the beat teaches
...
**Read each value back in `CoachingResponse.fields_captured` under exactly
these names** (§20) — it is held as pending, and stored only when the Belt
confirms it (v1.75). Every turn also populates `explanation`,
...
> **What each field carries:** ① **Explain** (plain language, why it
> matters) · ② **Show** (worked example, visually distinct, illustration only)
> · ③ **Ask** (invite the Belt's version) · ④ **Read back** (their answer
> reflected back, then *"Is this right?"*). Which of the four a turn uses is
> decided in code, never here. Tone: warm, encouraging, never gatekeeping. Assume a capable but
...
> **Read back:** their reason, in their own words — *"Is this right?"*
...
> **Read back:** each person — name, role and function — *"Is this right?"*
...
> **Read back:** their customers and needs, in their own words — *"Is this right?"*
...
> **Ask:** What's happening? · Where? · When / since when? · Who's affected? · Why does it matter? · **How much — and is it one measure or more than one?** (a rough number each; many projects track both a quality measure and a time measure) · What would "fixed" look like?
> **Read back (composed):** Putting that together, here's your problem statement: *"[composed from the Belt's own answers]"* — does that capture it accurately? *(Guard: assemble only what the Belt said; invent nothing.)*
...
**For each metric the Belt names, the teaching says three things:**
...
(§13). Woven into field 5's coaching, never delivered as a lecture.
...
> **Read back:** each metric by **name, unit, meaning and current value**, one per sentence — *"Is this right?"*

> **The read-back names the full metric set.** When the Belt names
> more than one metric, **the read-back lists them all and asks whether the set
> is complete** — *"So we're tracking two: error rate in %, and cycle time in days.
...
> **Read back:** one target per baseline measure — **every criterion in the baseline has a target and no target names a criterion the baseline didn't**; the gate checks this by name and unit, and a mismatch there is what a missing target looks like one phase later — *"Is this right?"*

**[TOOL · calculate_expected_savings · on the baseline and the target]**
...
**[11 · process_map_sipoc · required · built from a shown example]**
...
> **Ask:** Let's build yours: the **Process** — the 5–7 main steps, start to end — and its Suppliers, Inputs, Outputs and Customers. And: what do you measure on those outputs? (that's the sixth key, `process_metrics`). You can also upload a SIPOC if you have one.
...
field.** Define is the phase where Belts most often
...
**A structured field is read back whole, never key by key.** A
`process_map_sipoc` with four of six keys filled is the partial-map failure the
grader exists to catch (§41) — show the Belt which keys are still thin.

**`calculate_expected_savings` results land in
`artifacts["computation_results"]` automatically** — do not capture them as a
field. The Belt's own framing of the figure belongs in `business_case`.
