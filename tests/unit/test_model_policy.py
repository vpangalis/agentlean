...
def test_resolve_model_operational_default() -> None:
    policy = ModelPolicy(ModelStrategy(MockSettings()))
...
def test_model_strategy_reads_from_settings() -> None:
    strategy = ModelStrategy(MockSettings())
    assert strategy.intent_default == "intent-model"
    assert strategy.operational_default == "operational-model"
    assert strategy.operational_premium == "operational-premium"